...
import { getOfficialProductsByCategory } from "@/lib/mock-data/officialCatalog";
...
export default async function GrappStoreCategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const products = getOfficialProductsByCategory(slug as ProductCategory);

  return (
    <div>
      <div className="flex items-center gap-2 px-3 md:px-5 pt-3.5 pb-3">
        <Link href="/grappstore/categories" className="active:opacity-60 transition-opacity">
      ...
      {products.length === 0 ? (
        <div className="px-3 md:px-5 py-10 text-center text-[11px] text-gl-text-secondary">
          No GrappStore products in this category yet.
        </div>
      ) : (
